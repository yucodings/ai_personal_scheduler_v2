...
import { AlertCircle, ArrowLeft, Bot, Check, CheckCircle2, ChevronRight, FileText, GitBranch, History, LayoutList, Loader2, MessageSquareText, Pencil, Plus, RotateCcw, SlidersHorizontal, Sparkles, UploadCloud } from "lucide-react";
...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  const backHref = project.status === "archived" ? "/archived" : "/projects";
  return <div className="space-y-6">
    <div><Link href={backHref} className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-slate-950"><ArrowLeft className="h-4 w-4" />{project.status === "archived" ? "Archived projects" : "All projects"}</Link><div className="mt-4 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap items-center gap-2"><Badge tone={project.riskStatus === "on_track" ? "success" : project.riskStatus === "at_risk" ? "warning" : "danger"}>{project.riskStatus.replaceAll("_", " ")}</Badge><Badge>{project.projectType.replaceAll("_", " ")}</Badge>{project.isActiveContext ? <Badge tone="info">Active AI context</Badge> : null}</div><h1 className="mt-3 text-3xl font-semibold tracking-tight">{project.title}</h1><p className="mt-2 max-w-3xl text-slate-500">{project.description || "No project description yet."}</p></div><div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={() => setEditOpen(true)}><Pencil className="h-4 w-4" />Edit project</Button><Button variant="secondary" onClick={() => setTab("Files & knowledge")}><UploadCloud className="h-4 w-4" />Add file</Button><Button onClick={() => setTab("AI analysis")}><Sparkles className="h-4 w-4" />Analyse project</Button></div></div></div>
    ...
    {tab === "Tasks" && <Tasks projectId={projectId} tasks={projectTasks} onUpdate={updateTask} />}
...
function Tasks({ projectId, tasks, onUpdate }: { projectId: string; tasks: Task[]; onUpdate(id: string, progress: number, status?: Task["status"], blocker?: string): Promise<void> }) {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Work breakdown</h2><p className="mt-1 text-sm text-slate-500">Add work manually or approve an AI-generated plan.</p></div><div className="flex gap-2">{selected.length > 0 ? <Button size="sm" variant="secondary" onClick={() => void bulkComplete()}><Check className="h-4 w-4" />Complete {selected.length}</Button> : null}<Button size="sm" onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" />Add task</Button></div></div>{tasks.length ? Object.entries(grouped).map(([group, entries]) => <Card key={group}><CardHeader><h3 className="font-semibold">{group === "Unassigned" ? group : "Milestone tasks"}</h3><Badge tone="info">{entries?.length ?? 0} tasks</Badge></CardHeader><CardContent className="space-y-2">{entries?.map((task) => <div key={task.id} className="grid gap-3 rounded-xl border border-slate-100 p-3 md:grid-cols-[auto_1fr_120px_140px] md:items-center"><input type="checkbox" aria-label={`Select ${task.title}`} checked={selected.includes(task.id)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, task.id] : current.filter((id) => id !== task.id))} /><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{task.title}</p><Badge tone={statusTone[task.status]}>{task.status.replaceAll("_", " ")}</Badge>{task.dependencies.length > 0 ? <span className="inline-flex items-center gap-1 text-xs text-slate-400"><GitBranch className="h-3 w-3" />{task.dependencies.length}</span> : null}</div><p className="mt-1 text-xs text-slate-500">{task.estimatedHours}h estimate{task.dueDate ? ` · due ${formatDate(task.dueDate)}` : ""}</p></div><div><Progress value={task.progressPercent} /><p className="mt-1 text-right text-xs text-slate-400">{task.progressPercent}%</p></div><select aria-label={`Update ${task.title} status`} className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs" value={task.status} onChange={(event) => { const status = event.target.value as Task["status"]; void onUpdate(task.id, status === "completed" ? 100 : status === "not_started" ? 0 : task.progressPercent || 25, status, status === "blocked" ? task.blockedReason ?? "Blocked from the web workspace" : undefined); }}><option value="not_started">Not started</option><option value="in_progress">In progress</option><option value="nearly_complete">Nearly complete</option><option value="completed">Completed</option><option value="blocked">Blocked</option></select></div>)}</CardContent></Card>) : <Empty icon={LayoutList} title="No tasks yet" detail="Use Add task for manual work, or run AI analysis after uploading project files." />}<CreateTaskDialog projectId={projectId} sequence={tasks.length} open={createOpen} onOpenChange={setCreateOpen} /></div>;
}
