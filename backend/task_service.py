...
class TaskService:
    ...
    def create(self, data: TaskInput):
        task = self.db.table("tasks", method="POST", data=data.model_dump(mode="json"), prefer="return=representation")[0]
        self.db.rpc("recalculate_project_progress", {"p_project_id": str(data.project_id)})
        return task
