...
export default function ProjectsPage() {
    ...
    .sort((a, b) => sort === "deadline" ? a.finalDeadline.localeCompare(b.finalDeadline)
      : sort === "progress" ? b.displayedProgress - a.displayedProgress
        : sort === "priority" ? ["critical", "high", "medium", "low"].indexOf(a.priority) - ["critical", "high", "medium", "low"].indexOf(b.priority)
          : b.updatedAt.localeCompare(a.updatedAt)), [projects, query, risk, status, sort]);
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 lg:flex-row">
      ...
      <div className="flex flex-wrap gap-2"><div className="flex items-center gap-2 px-2 text-sm text-slate-400"><Filter className="h-4 w-4" />Filters</div><Select aria-label="Status filter" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All status</option><option value="active">Active</option><option value="planned">Planned</option><option value="paused">Paused</option><option value="archived">Archived</option></Select><Select aria-label="Risk filter" value={risk} onChange={(event) => setRisk(event.target.value)}><option value="all">All risk</option><option value="on_track">On track</option><option value="at_risk">At risk</option><option value="delayed">Delayed</option><option value="blocked">Blocked</option></Select><Select aria-label="Sort projects" value={sort} onChange={(event) => setSort(event.target.value)}><option value="deadline">Deadline</option><option value="progress">Progress</option><option value="priority">Priority</option><option value="updated">Recently updated</option></Select></div>
    </div>
    ...
    {!loading && !error && filtered.length === 0 && <div className="rounded-3xl border border-dashed border-slate-300 py-20 text-center"><p className="font-semibold">{projects.length ? "No projects match these filters." : "Your workspace is empty."}</p><p className="mt-1 text-sm text-slate-500">{projects.length ? "Try clearing a filter." : "Create your first real project; it will be saved to Supabase."}</p></div>}
